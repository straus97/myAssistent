#!/usr/bin/env python3
"""
Агрессивная улучшенная модель торговли v2.1
Цель: +5-10% return, Sharpe > 1.0

Улучшения:
1. Более агрессивное позиционирование
2. Улучшенные фильтры входа
3. Лучшие пороги сигналов
4. Оптимизация размера позиций
"""
...
def load_historical_data(exchange='bybit', symbol='BTC/USDT', timeframe='1h', limit=2000):
    """Загружаем больше исторических данных для обучения"""
...
def create_advanced_features(df):
    """Создаем продвинутые технические индикаторы"""
    print("Создаем продвинутые технические индикаторы...")
    ...
    # ADX (упрощенный)
    df['adx_14'] = 50  # Упрощенно, в реальности нужен сложный расчет
    ...
    # Target variable (1 if future return > 0.002, 0 otherwise) - более строгий порог
    df['y'] = (df['future_ret'] > 0.002).astype(int)
...
def aggressive_position_sizing(df, model, threshold):
    """Более агрессивное позиционирование"""
    print("Рассчитываем агрессивные размеры позиций...")
    
    feature_cols = [col for col in df.columns if col not in ['future_ret', 'y', 'open', 'high', 'low', 'close', 'volume']]
    X = df[feature_cols].fillna(0)
    y_proba = model.predict_proba(X)[:, 1]
    
    # Более агрессивный базовый размер
    base_position_size = 0.15  # 15% от капитала
    
    # Адаптивный размер на основе волатильности (менее консервативный)
    atr_pct = df.get('atr_pct', df['ret_1'].rolling(14).std()).fillna(0.02)
    
    # Менее агрессивное уменьшение при высокой волатильности
    volatility_adjustment = 1 / (1 + atr_pct * 5)  # Было 10, стало 5
    
    # Размер на основе уверенности модели (более агрессивный)
    confidence_adjustment = np.where(
        y_proba > threshold,
        (y_proba - threshold) / (1 - threshold) * 1.5,  # Увеличиваем в 1.5 раза
        0
    )
    ...
    # Более агрессивные ограничения
    position_sizes = np.clip(position_sizes, 0.05, 0.4)  # 5-40% от капитала
...
def optimized_signal_thresholds(df, model):
    """Оптимизированные пороги сигналов для агрессивной торговли"""
    print("Оптимизируем пороги сигналов для агрессивной торговли...")
    ...
    # Тестируем разные пороги (более низкие для агрессивной торговли)
    thresholds = np.arange(0.45, 0.75, 0.05)
    signal_quality = []
    
    for threshold in thresholds:
        signals = y_proba > threshold
        if signals.sum() < 20:  # Минимум 20 сигналов
            continue
        ...
        # Более агрессивная метрика качества (больше веса на количество сигналов)
        quality = win_rate * avg_return * (signals.sum() / len(df)) * 2
    ...
    if signal_quality:
        ...
        print("WARNING: Не найдено подходящих порогов, используем 0.5")
        return 0.5

def aggressive_backtest(df, model, threshold, position_sizes):
    """Агрессивный бэктест с увеличенными размерами позиций"""
    print("Запускаем агрессивный бэктест...")
    
    feature_cols = [col for col in df.columns if col not in ['future_ret', 'y', 'open', 'high', 'low', 'close', 'volume']]
    X = df[feature_cols].fillna(0)
    y_proba = model.predict_proba(X)[:, 1]
    
    # Сигналы с оптимизированным порогом
    signals = y_proba > threshold
    
    # Агрессивная симуляция бэктеста
...
def main():
    """Основная функция агрессивного улучшения модели"""
    print("MyAssistent Aggressive Model Improvement v2.1")
    print("=" * 50)
    
    # 1. Загружаем больше данных
    print("\n1. Загружаем расширенные исторические данные...")
    df_prices = load_historical_data(limit=2000)  # 2000 свечей
    
    if df_prices is None:
        print("ОШИБКА: Не удалось загрузить данные")
        return
    
    # 2. Создаем продвинутые фичи
    print("\n2. Создаем продвинутые технические индикаторы...")
    df = create_advanced_features(df_prices)
    ...
    print("\n3. Обучаем агрессивную модель...")
    feature_cols = [col for col in df.columns if col not in ['future_ret', 'y', 'open', 'high', 'low', 'close', 'volume']]
    
    # Улучшенное обучение XGBoost
    ...
    # Более агрессивные параметры XGBoost
    ...
    # 4. Оптимизируем пороги для агрессивной торговли
    threshold = optimized_signal_thresholds(df, model)
    
    # 5. Рассчитываем агрессивные размеры позиций
    position_sizes = aggressive_position_sizing(df, model, threshold)
    
    # 6. Запускаем агрессивный бэктест
    results = aggressive_backtest(df, model, threshold, position_sizes)
    
    # 7. Сохраняем результаты
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = f"artifacts/backtest/aggressive_backtest_{timestamp}.json"
    ...
    print("\n" + "=" * 50)
    print("РЕЗУЛЬТАТЫ АГРЕССИВНОЙ МОДЕЛИ v2.1")
    print("=" * 50)
