#!/usr/bin/env python3
"""
Улучшенная модель торговли v2.0
...
def improved_backtest(df, model, threshold, position_sizes):
    ...
    results = {
        ...
        'trades': trades[:50],  # Ограничиваем количество сделок для JSON
        'benchmark': {
            'benchmark_return': benchmark_total_return,
...
def main():
    ...
    print("\n1. Загружаем расширенный датасет...")
    df_prices = load_historical_data(limit=4320)  # 6 месяцев данных
    
    if df_prices is None:
        print("ОШИБКА: Не удалось загрузить данные")
        return
    
    # 2. Строим датасет с фичами
    print("\n2. Строим датасет с фичами...")
    # Используем данные напрямую, так как build_dataset работает с БД
    df = df_prices.copy()
    
    # Добавляем базовые фичи для демонстрации
    ...
    if results.get('success'):
        ...
        print(f"   Return ≥ {target_return*100}%: {'✅' if return_achieved else '❌'} ({metrics['total_return']*100:.2f}%)")
        print(f"   Sharpe ≥ {target_sharpe}: {'✅' if sharpe_achieved else '❌'} ({metrics['sharpe_ratio']:.4f})")
